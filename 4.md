Awesome 🚀
Now we’re entering **real-world React** territory.

What we’ll do **now** is the **MOST IMPORTANT jump** from “I know React” → “I can work on production apps”.

---

# 🎯 TODAY’S GOAL (VERY CLEAR)

We will add **REAL async behavior** properly:

✅ `useEffect`
✅ API layer (clean separation)
✅ loading & error states
✅ async flow using **the SAME reducer + context pattern**

⚠️ No Redux yet — because **this is how Redux logic actually works internally**.

---

## 🧠 First: The BIG IDEA (Before Code)

In production:

❌ UI should NOT call `fetch()` directly
❌ UI should NOT handle business logic

✅ UI should say **WHAT happened**
✅ Logic should live **outside components**

So we introduce a **service layer**.

---

# 🚀 STEP 1 — Create API Service (Fake Backend)

### 📁 `src/features/todos/todo.api.ts`

```ts
import { Todo } from "./todo.types";

let fakeTodos: Todo[] = [
  { id: 1, text: "Learn async flow", completed: false },
  { id: 2, text: "Understand reducer pattern", completed: true },
];

export function fetchTodos(): Promise<Todo[]> {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve(fakeTodos);
    }, 800);
  });
}

export function createTodo(text: string): Promise<Todo> {
  return new Promise((resolve) => {
    setTimeout(() => {
      const newTodo = {
        id: Date.now(),
        text,
        completed: false,
      };
      fakeTodos.push(newTodo);
      resolve(newTodo);
    }, 500);
  });
}
```

### 🔑 Why this exists

* Simulates backend
* Can be replaced by real API later
* UI doesn’t care where data comes from

---

# 🚀 STEP 2 — Extend State & Actions

### 📁 `todo.types.ts` (UPDATED)

```ts
export type State = {
  todos: Todo[];
  loading: boolean;
  error: string | null;
};

export type Action =
  | { type: "FETCH_START" }
  | { type: "FETCH_SUCCESS"; payload: Todo[] }
  | { type: "FETCH_ERROR"; payload: string }
  | { type: "ADD_TODO"; payload: Todo };
```

### Why this matters

This is **classic async state**:

```
start → success | error
```

Every production app uses this.

---

# 🚀 STEP 3 — Reducer Handles ASYNC STATES

### 📁 `todo.reducer.ts` (UPDATED)

```ts
import { State, Action } from "./todo.types";

export function todoReducer(state: State, action: Action): State {
  switch (action.type) {
    case "FETCH_START":
      return { ...state, loading: true, error: null };

    case "FETCH_SUCCESS":
      return { ...state, loading: false, todos: action.payload };

    case "FETCH_ERROR":
      return { ...state, loading: false, error: action.payload };

    case "ADD_TODO":
      return { ...state, todos: [...state.todos, action.payload] };

    default:
      return state;
  }
}
```

### 🔥 IMPORTANT

Reducer **does not know async exists**
It only reacts to actions.

This is **clean architecture**.

---

# 🚀 STEP 4 — Trigger Async Logic in Provider (KEY STEP)

### 📁 `todo.context.tsx` (UPDATED)

```tsx
import React, { useEffect } from "react";
import { todoReducer } from "./todo.reducer";
import { fetchTodos, createTodo } from "./todo.api";
import { State, Action } from "./todo.types";

type TodoContextType = {
  state: State;
  loadTodos: () => void;
  addTodo: (text: string) => void;
};

export const TodoContext =
  React.createContext<TodoContextType | null>(null);

export function TodoProvider({ children }: { children: React.ReactNode }) {
  const [state, dispatch] = React.useReducer(todoReducer, {
    todos: [],
    loading: false,
    error: null,
  });

  const loadTodos = async () => {
    dispatch({ type: "FETCH_START" });
    try {
      const todos = await fetchTodos();
      dispatch({ type: "FETCH_SUCCESS", payload: todos });
    } catch {
      dispatch({ type: "FETCH_ERROR", payload: "Failed to load todos" });
    }
  };

  const addTodo = async (text: string) => {
    const todo = await createTodo(text);
    dispatch({ type: "ADD_TODO", payload: todo });
  };

  useEffect(() => {
    loadTodos();
  }, []);

  return (
    <TodoContext.Provider value={{ state, loadTodos, addTodo }}>
      {children}
    </TodoContext.Provider>
  );
}
```

### 🚨 THIS IS HUGE

We moved:

* async logic ❌ out of UI
* into Provider ✅

This is **exactly how Redux Thunks work**.

---

# 🚀 STEP 5 — Update Hook

### 📁 `todo.hooks.ts`

```ts
import { useContext } from "react";
import { TodoContext } from "./todo.context";

export function useTodos() {
  const context = useContext(TodoContext);
  if (!context) {
    throw new Error("useTodos must be used inside TodoProvider");
  }
  return context;
}
```

---

# 🚀 STEP 6 — Components Become VERY SIMPLE 😍

### 📁 `AddTodo.tsx`

```tsx
import { useState } from "react";
import { useTodos } from "../todo.hooks";

export function AddTodo() {
  const [text, setText] = useState("");
  const { addTodo } = useTodos();

  return (
    <div>
      <input value={text} onChange={(e) => setText(e.target.value)} />
      <button onClick={() => addTodo(text)}>Add</button>
    </div>
  );
}
```

---

### 📁 `TodoList.tsx`

```tsx
import { useTodos } from "../todo.hooks";

export function TodoList() {
  const { state } = useTodos();

  if (state.loading) return <p>Loading...</p>;
  if (state.error) return <p>{state.error}</p>;

  return (
    <ul>
      {state.todos.map((todo) => (
        <li key={todo.id}>{todo.text}</li>
      ))}
    </ul>
  );
}
```

---

# 🧠 WHAT YOU JUST LEARNED (BIG DEAL)

You now understand:

✅ Real async state handling
✅ Clean separation of concerns
✅ Why reducers scale
✅ How Redux Toolkit works internally
✅ How professional React apps are built

This is **intermediate → advanced React**.

---

# 🏆 NEXT LEVEL (I decide again)

Next best step:

> 🔥 **Redux Toolkit (RTK)**

Why?

* You already think in reducers
* RTK will feel EASY now
* This is what companies actually use

OR we can:
1️⃣ Add **authentication flow**
2️⃣ Build **Dashboard app**
3️⃣ Convert this to **Redux Toolkit**
4️⃣ Add **testing (Jest)**

👉 Just reply with the number
You’re doing AMAZING 👏
